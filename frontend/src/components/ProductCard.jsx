  ...
  return (
    <article className="group flex min-w-0 flex-col overflow-hidden rounded-2xl border border-lime-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg hover:shadow-lime-100">
      <Link to={`/product/${product._id}`} className="flex flex-1 cursor-pointer flex-col">
        <div className={`relative grid place-items-center overflow-hidden bg-lime-50/60 p-2.5 sm:p-3 ${compact ? "h-32 sm:h-36" : "h-40 sm:h-48"}`}>
        ...
        <div className="flex flex-1 flex-col px-3 pt-3">
          ...
          <h3 className="mt-1 line-clamp-2 min-h-10 text-sm font-semibold text-slate-800 group-hover:text-lime-700">{displayName}</h3>
      ...
      <div className="px-3 pb-3 pt-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <select
            ...
            className="min-w-24 flex-1 rounded-xl border border-lime-200 bg-lime-50 px-2 py-2 text-[11px] font-bold text-lime-800 outline-none disabled:border-slate-200 disabled:bg-slate-50 disabled:text-slate-400"
            aria-label={`${displayName} variant`}
          >
          ...
          {quantity > 0 ? (
            ...
            <button type="button" onClick={handleAdd} disabled={isOutOfStock} className="shrink-0 rounded-xl border border-lime-600 bg-white px-3 py-2 text-xs font-extrabold text-lime-700 transition hover:bg-lime-50 disabled:border-slate-200 disabled:text-slate-400 sm:px-4">{isPreviewOnly ? "PREVIEW" : "ADD"}</button>
          )}
        </div>
      </div>
    </article>
